defmodule Digger.AtomizerTest do
  ...
  test "can atomize nested string keys" do
    ...
    assert Digger.Atomizer.atomize(stringified_map) == %{%{:c => 4, :d => 5} => 6, :a => 2, :b => 3}
  end
  ...
  test "does not atomize char lists, lists, or structs like date" do
    ...
    stringified_map = %{%{"e" => 4, :f => 5} => 6, [1, 2] => 2,
                        %{"c" => 3, 'ab' => 4, date => 1} => 7}
    assert Digger.Atomizer.atomize(stringified_map) == %{%{:e => 4, :f => 5} => 6, [1, 2] => 2,
                                                         %{:c => 3, 'ab' => 4, date => 1} => 7}
  end

  test "can atomize the empty string, as needed" do
    stringified_map = %{%{"e" => 4, :f => "5"} => 6, "" => [1, 2]}
    assert Digger.Atomizer.atomize(stringified_map) == %{%{:e => 4, :f => "5"} => 6, :"" => [1, 2]}
  end
